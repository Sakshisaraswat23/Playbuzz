import User from '../model/Match_model.js';

// Get all users
export const getMatches = async (request, response) => {
	try {
...
// Get a user by id
export const getMatchesById = async (request, response) => {
	try {
...
//add data
export const addMatches = async (request, response) => {
...
export const editScore = async (request, response) => {
	try {
...
export const matchFiltersController = async (req, res) => {
	try {
		...
		if (checked.length > 0) args.sports = checked;
		if (radio.length) args.gender = radio;
		const matches = await User.find(args).sort({ date: -1 });
		//   console.log(matches);
...
// module.exports = { getMatches, getMatchesById, editScore,addMatches };
